...
import { Plus, Search, Mail, FolderOpen, LayoutGrid, Sparkles, Presentation, Download, Eye, ChevronDown, ChevronUp, FileText, BookOpen, FileDown } from 'lucide-react';
...
export default function ContentLibraryPage() {
  ...
  const [downloadingVendorBrief, setDownloadingVendorBrief] = useState(false);
  ...
  const handleExportVendorBrief = useCallback(async () => {
    setDownloadingVendorBrief(true);
    try {
      const res = await fetch('/api/ai/generate-vendor-brief', { method: 'POST' });
      if (!res.ok) throw new Error('Failed to generate vendor brief');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'FFA_North_Content_Library_and_Production_Guidelines.pdf';
      a.click();
      URL.revokeObjectURL(url);
      showToast('Vendor brief PDF downloaded');
    } catch {
      showToast('Failed to generate vendor brief');
    } finally {
      setDownloadingVendorBrief(false);
    }
  }, [showToast]);
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 animate-fade-up">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handleExportVendorBrief}
            disabled={downloadingVendorBrief}
            className="border border-amber-200 text-amber-700 bg-amber-50 rounded-lg hover:bg-amber-100 px-4 py-2 text-sm font-semibold transition-colors inline-flex items-center gap-2 disabled:opacity-60"
          >
            {downloadingVendorBrief ? (
              <>
